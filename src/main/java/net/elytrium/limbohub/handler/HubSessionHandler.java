...
import net.elytrium.limbohub.protocol.packets.CloseContainerClient;
import net.elytrium.limbohub.protocol.packets.Interact;
import net.elytrium.limbohub.protocol.packets.PlayerAction;
...
import net.elytrium.limbohub.protocol.packets.SetCooldown;
import net.elytrium.limbohub.protocol.packets.SetHeldItem;
import net.elytrium.limbohub.protocol.packets.UseItem;
...
public class HubSessionHandler implements LimboSessionHandler {
  ...
  public void addItems() {
    Iterator var1 = Settings.IMP.MAIN.PLAYER_ITEMS.iterator();

    while(var1.hasNext()) {
      Settings.MAIN.ITEM_DATA player_item = (Settings.MAIN.ITEM_DATA)var1.next();
      List<Component> loreComponents = player_item.LORE == null ? null : (List)player_item.LORE.stream().map((line) -> {
        return LimboHub.buildResetComponent().append(LimboHub.getSerializer().deserialize(line));
      }).collect(Collectors.toList());
      ItemMeta meta = new StaticItemMeta(LimboHub.buildResetComponent().append(LimboHub.getSerializer().deserialize(player_item.CUSTOM_NAME)), loreComponents, player_item.HAS_COLOR, player_item.COLOR, player_item.ENCHANTED, player_item.SKULL_OWNER);
      CompoundBinaryTag nbt = meta.buildNbt(this.proxyPlayer.getProtocolVersion());
      VirtualItem item = this.plugin.getLimboFactory().getItem(player_item.ITEM);
      this.player.setInventory(player_item.SLOT, item, player_item.COUNT, player_item.DATA, nbt);
    }
  ...
  public void onGeneric(Object packet) {
    if (packet instanceof ClickContainer) {
      ...
      if (this.currentMenu == null) {
        if (clickContainer.getWindowId() == 0) {
          Iterator var14 = Settings.IMP.MAIN.PLAYER_ITEMS.iterator();

          while (true) {
            Settings.MAIN.ITEM_DATA _item;
            do {
              do {
                if (!var14.hasNext()) {
                  return;
                ...
                _item = (Settings.MAIN.ITEM_DATA) var14.next();
              } while (_item.SLOT != clickContainer.getSlot());
              ...
              this.addItems();
            } while (Settings.IMP.MAIN.ITEM_COOLDOWN < 50 && Settings.IMP.MAIN.ITEM_COOLDOWN % 50 > 0 && Settings.IMP.MAIN.ITEM_COOLDOWN == 0);

            if (this.isCooldown == 0) {
              this.player.writePacketAndFlush(new CloseContainerClient(clickContainer.getWindowId()));
              this.handleAction(_item.ACTION);
              this.isCooldown = 1;
              if (this.isCooldown == 1) {
                this.player.writePacketAndFlush(new SetCooldown(this.plugin.getLimboFactory().getItem(_item.ITEM).getID(this.proxyPlayer.getProtocolVersion()), Settings.IMP.MAIN.ITEM_COOLDOWN / 50));
                this.isCooldown = 2;
                if (this.isCooldown == 2) {
                  this.plugin.getServer().getScheduler().buildTask(this.plugin, () -> {
                    this.isCooldown = 0;
                  }).delay((long) Settings.IMP.MAIN.ITEM_COOLDOWN, TimeUnit.MILLISECONDS).schedule();
                }
              }
            }
          }
        ...
        if (containerId != clickContainer.getWindowId() && clickContainer.getWindowId() == 0) {
          Iterator var5 = Settings.IMP.MAIN.PLAYER_ITEMS.iterator();

          while (var5.hasNext()) {
            Settings.MAIN.ITEM_DATA _item = (Settings.MAIN.ITEM_DATA) var5.next();
            if (_item.SLOT == clickContainer.getSlot()) {
              if (this.proxyPlayer.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_17) <= 0) {
                this.player.writePacketAndFlush(new SetContainerSlot(-1, -1, ItemStack.EMPTY));
              }
              ...
              this.player.writePacketAndFlush(new SetContainerSlot(0, 45, ItemStack.EMPTY));
              this.addItems();
              if ((Settings.IMP.MAIN.ITEM_COOLDOWN >= 50 || Settings.IMP.MAIN.ITEM_COOLDOWN % 50 <= 0 || Settings.IMP.MAIN.ITEM_COOLDOWN != 0) && this.isCooldown == 0) {
                this.player.writePacketAndFlush(new CloseContainerClient(clickContainer.getWindowId()));
                this.handleAction(_item.ACTION);
                this.isCooldown = 1;
                if (this.isCooldown == 1) {
    ...
    if (packet instanceof Interact) {
      ...
      Iterator var2 = Settings.IMP.MAIN.PLAYER_ITEMS.iterator();

      while(var2.hasNext()) {
        Settings.MAIN.ITEM_DATA _item = (Settings.MAIN.ITEM_DATA)var2.next();
        if (_item.SLOT == this.heldslot + 36 && this.isCooldown == 0) {
          this.handleAction(_item.ACTION);
          this.isCooldown = 1;
          if (this.isCooldown == 1) {
            this.player.writePacketAndFlush(new SetCooldown(this.plugin.getLimboFactory().getItem(_item.ITEM).getID(this.proxyPlayer.getProtocolVersion()), Settings.IMP.MAIN.ITEM_COOLDOWN / 50));
    ...
    } else if (packet instanceof SetHeldItem) {
      SetHeldItem heldItem = (SetHeldItem)packet;
      this.heldslot = heldItem.getSlot();
    } else if (packet instanceof PlayerAction) {
      PlayerAction action = (PlayerAction)packet;
      if (action.getStatus() == PlayerAction.Status.DROP_ITEM || action.getStatus() == PlayerAction.Status.DROP_ITEM_STACK) {
        this.player.writePacketAndFlush(new SetContainerContent((Container)null));
        this.addItems();
      }
    }
